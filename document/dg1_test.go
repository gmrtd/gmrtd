...
func TestNewDG1(t *testing.T) {
	testCases := []struct {
	...
	}{
		{
			...
			rawMrz: "I<UTOERIKSSON<<ANNA<MARIA<<<<<<<<<<<D231458907UTO7408122F1204159<<<<<<<6",
		},
		{
			...
			rawMrz: "P<SURNAME<<FIRSTNAME<<<<<<<<<<<<<<<<<<<<<<<<DOCNUM<<<8UTO8201054M3401011<<<<<<<<<<<<<<<6",
		},
	}
	for _, tc := range testCases {
		...
		if doc.Mf.Lds1.Dg1.RawMrz != tc.rawMrz {
			t.Errorf("DG1 RawMrz mismatch (Exp:%s, Act:%s)", tc.rawMrz, doc.Mf.Lds1.Dg1.RawMrz)
		}
	}
}
