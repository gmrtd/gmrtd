...
func TestDecodeX962EcPoint(t *testing.T) {
	p256 := elliptic.P256()
	p384 := elliptic.P384()

	validP256Uncompressed := elliptic.Marshal(p256, p256.Params().Gx, p256.Params().Gy)
	validP384Uncompressed := elliptic.Marshal(p384, p384.Params().Gx, p384.Params().Gy)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if !tt.curve.IsOnCurve(got.X, got.Y) {
				t.Fatal("decoded point is not on expected curve")
			}
		})
	}
}

func TestDecodeX962EcPointReturnsExpectedCoordinates(t *testing.T) {
	curve := elliptic.P256()
	wantX := curve.Params().Gx
	...
	if got.X == nil || got.X.Cmp(wantX) != 0 {
		t.Fatalf("unexpected X: got %v, want %v", got.X, wantX)
	}

	if got.Y == nil || got.Y.Cmp(wantY) != 0 {
		t.Fatalf("unexpected Y: got %v, want %v", got.Y, wantY)
	}
}
...
func TestRsaDecryptWithPublicKey(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			...
			expPlaintext: utils.HexToBytes(""),
		},
		{
			...
			expPlaintext: utils.HexToBytes(""),
		},
	}
	for _, tc := range testCases {
		var plaintext []byte
...
func TestRsaDecryptWithPublicKeyLeadingZeros(t *testing.T) {
	// This test verifies that leading zeros are preserved in RSA decryption results.
	// This is critical for PKCS#1 v1.5 padding validation, which requires fixed-width output.
	// Australian passports specifically require this fix for Active Authentication.

	// Use one of the existing test cases that demonstrates leading zero preservation
	ciphertext := utils.HexToBytes("")
	publicKey := RsaPublicKey{E: 65537, N: new(big.Int).SetBytes(utils.HexToBytes(""))}
	...
	// Verify that the result length matches the RSA key width
	if len(plaintext) != keyWidth {
		t.Errorf("Result length mismatch (Exp:%d, Act:%d) - leading zeros not preserved", keyWidth, len(plaintext))
	}

	// Verify that the result starts with leading zero bytes (PKCS#1 v1.5 padding: 0x00 0x01)
	if len(plaintext) < 2 {
		t.Errorf("Result too short")
	}
	if plaintext[0] != 0x00 {
		t.Errorf("First byte should be 0x00 (leading zero), got 0x%02x - leading zeros stripped", plaintext[0])
	}
	if plaintext[1] != 0x01 {
		t.Errorf("Second byte should be 0x01 (PKCS#1 v1.5 padding), got 0x%02x", plaintext[1])
	}
}
