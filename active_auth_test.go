package gmrtd

import (
	"bytes"
	"crypto"
	"reflect"
	"testing"
)

func TestDoActiveAuth(t *testing.T) {
	// "doActiveAuth","SM(pre)":"(alg:1, ksenc:b99d546108eaa251570876b6d3456dce, ksmac:e3857ca24946251c151c540e13f2cd51, ssc:00000000000000cc)"}
	//
	// DG15: 
	// RND.IFD: 96302b0f3d7e7864
	//
	// cAPDU: 0c88000020871101ed7f8cb47a4eea086324a7f9dd7427809701008e0897f54bae49aa71f800
	// rAPDU: 
	//
	// AA enabled

	var err error

	var nfc *NfcSession

	{
		var transceiver *MockTransceiver = new(MockTransceiver)

		// add in expected request/response tuples
		transceiver.AddReqRsp("0c88000020871101ed7f8cb47a4eea086324a7f9dd7427809701008e0897f54bae49aa71f800", "")

		nfc = NewNfcSession(transceiver)
	}

	// setup SM
	{
		nfc.sm, err = NewSecureMessaging(TDES, HexToBytes("b99d546108eaa251570876b6d3456dce"), HexToBytes("e3857ca24946251c151c540e13f2cd51"))
		if err != nil {
			t.Errorf("Unexpected error: %s", err)
		}

		nfc.sm.SetSSC(HexToBytes("00000000000000cc"))
	}

	// setup static randoms for test
	getTestRandomBytesFn := func() func(length int) []byte {
		var idx int

		return func(length int) []byte {
			var out []byte

			switch idx {
			case 0:
				out = HexToBytes("96302b0f3d7e7864")
			default:
				t.Errorf("Invalid index (idx:%1d)", idx)
			}

			// sanity check that length matches requested amount
			if len(out) != length {
				t.Errorf("Test data length does NOT match amount requested (req:%d, act:%d)", length, len(out))
			}

			idx++

			return out
		}
	}

	var doc Document

	var dg15bytes []byte = HexToBytes("")

	err = doc.NewDG(15, dg15bytes)
	if err != nil {
		t.Errorf("Unexpected error: %s", err)
	}

	var activeAuth *ActiveAuth = NewActiveAuth()

	activeAuth.randomBytesFn = getTestRandomBytesFn()

	err = activeAuth.doActiveAuth(nfc, &doc)
	if err != nil {
		t.Errorf("Unexpected error: %s", err)
	}

	if doc.ChipAuthStatus != CHIP_AUTH_STATUS_AA {
		t.Errorf("Incorrect ChipAuth State (Exp:%1d, Act:%01d)", CHIP_AUTH_STATUS_AA, doc.ChipAuthStatus)
	}

	// verify Secure-Messaging post-state is correct
	// NB active-auth does NOT setup a new SM, do we just sanity check that it's still there with the correct SSC (ie +2)
	{
		smExp, err := NewSecureMessaging(TDES, HexToBytes("b99d546108eaa251570876b6d3456dce"), HexToBytes("e3857ca24946251c151c540e13f2cd51"))
		if err != nil {
			t.Errorf("Unexpected error: %s", err)
		}

		smExp.SetSSC(HexToBytes("00000000000000ce"))

		if !reflect.DeepEqual(smExp, nfc.sm) {
			t.Errorf("SecureMessaging differs to expected")
		}
	}

}

func TestDecodeF(t *testing.T) {
	testCases := []struct {
		rndIfd     []byte
		f          []byte
		expM1      []byte
		expD       []byte
		expHashAlg crypto.Hash
	}{
		{
			// SHA-1
			rndIfd:     HexToBytes("0102030405060708"),
			f:          HexToBytes("6A1234567890ABCDEF1234567890ABCDEF11223344e9b287952a913cc5b60ff74ecfc87e40e81563b1BC"),
			expM1:      HexToBytes("1234567890ABCDEF1234567890ABCDEF11223344"),
			expD:       HexToBytes("e9b287952a913cc5b60ff74ecfc87e40e81563b1"),
			expHashAlg: crypto.SHA1,
		},
		{
			// SHA-224
			rndIfd:     HexToBytes("0102030405060708"),
			f:          HexToBytes("6A1234567890ABCDEF1234567890ABCDEF11223344aefc05e88f48eb88d367ba3dacb7d6a8543ec02bd315e47ddbe2045d38CC"),
			expM1:      HexToBytes("1234567890ABCDEF1234567890ABCDEF11223344"),
			expD:       HexToBytes("aefc05e88f48eb88d367ba3dacb7d6a8543ec02bd315e47ddbe2045d"),
			expHashAlg: crypto.SHA224,
		},
		{
			// SHA-256
			rndIfd:     HexToBytes("0102030405060708"),
			f:          HexToBytes("6A1234567890ABCDEF1234567890ABCDEF11223344783980d44fd3a80f0e4210eb9c73ea399932062d465438f8e1ff13377de8308934CC"),
			expM1:      HexToBytes("1234567890ABCDEF1234567890ABCDEF11223344"),
			expD:       HexToBytes("783980d44fd3a80f0e4210eb9c73ea399932062d465438f8e1ff13377de83089"),
			expHashAlg: crypto.SHA256,
		},
		{
			// SHA-384
			rndIfd:     HexToBytes("0102030405060708"),
			f:          HexToBytes("6A1234567890ABCDEF1234567890ABCDEF112233448e0873b0baf595a777b497779981bd94dda3a81fd61cf2d526ab490f2d6cbcecc6eab5804df4e0a70169d4d0d6c07e7536CC"),
			expM1:      HexToBytes("1234567890ABCDEF1234567890ABCDEF11223344"),
			expD:       HexToBytes("8e0873b0baf595a777b497779981bd94dda3a81fd61cf2d526ab490f2d6cbcecc6eab5804df4e0a70169d4d0d6c07e75"),
			expHashAlg: crypto.SHA384,
		},
		{
			// SHA-512
			rndIfd:     HexToBytes("0102030405060708"),
			f:          HexToBytes("6A1234567890ABCDEF1234567890ABCDEF11223344968304e09f0c0ec86be4ade4b82d97d04283e3652b61193856c9ede1dac8962b3da9580fd77e9f9ef18a24c517d8d3b05f4eeb177e990ae1e80895c8bb28f51635CC"),
			expM1:      HexToBytes("1234567890ABCDEF1234567890ABCDEF11223344"),
			expD:       HexToBytes("968304e09f0c0ec86be4ade4b82d97d04283e3652b61193856c9ede1dac8962b3da9580fd77e9f9ef18a24c517d8d3b05f4eeb177e990ae1e80895c8bb28f516"),
			expHashAlg: crypto.SHA512,
		},
	}
	for _, tc := range testCases {
		m1, d, hashAlg, err := decodeF(tc.f)
		if err != nil {
			t.Errorf("Unexpected error: %s", err)
		}

		if !bytes.Equal(m1, tc.expM1) {
			t.Errorf("Incorrect m1 (Exp:%x) (Act:%x)", tc.expM1, m1)
		}

		if !bytes.Equal(d, tc.expD) {
			t.Errorf("Incorrect d (Exp:%x) (Act:%x)", tc.expD, d)
		}

		if hashAlg != tc.expHashAlg {
			t.Errorf("Incorrect hashAlg (Exp:%1d) (Act:%1d)", tc.expHashAlg, hashAlg)
		}

		// sanity check that hash works
		// NB this also verifies that the underlying hashAlg is linked
		{
			var m []byte = bytes.Clone(m1)
			m = append(m, tc.rndIfd...)

			calcD := CryptoHash(hashAlg, m)

			if !bytes.Equal(calcD, d) {
				t.Errorf("Hash sanithy-check failed for constructed 'm' (Exp:%x) (Act:%x)", calcD, d)
			}
		}
	}
}

func TestDecodeFerrors(t *testing.T) {
	testCases := []struct {
		f []byte
	}{
		{
			// no-data
			f: nil,
		},
		{
			// bad header byte - not 0x6A
			f: HexToBytes("6B000000"),
		},
		{
			// bad trailer byte (last byte)
			f: HexToBytes("6A000000"),
		},
		{
			// bad trailer byte (2nd last byte)
			f: HexToBytes("6A0000CC"),
		},
		{
			// sha-1, but too short to even extract the digest (d)
			f: HexToBytes("6Ab287952a913cc5b60ff74ecfc87e40e81563b1BC"),
		},
	}
	for _, tc := range testCases {
		m1, d, hashAlg, err := decodeF(tc.f)

		if err == nil {
			t.Errorf("Error expected")
		}

		if len(m1) > 0 || len(d) > 0 || hashAlg != 0 {
			t.Errorf("Defaults expected for m1(%x),d(%x),hashAlg(%d)", m1, d, hashAlg)
		}
	}

}
